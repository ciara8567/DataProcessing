...
rMax = max([Recovery1(siz1), Recovery2(siz2), Recovery3(siz3)]) + 0.5;
...
set(gca, 'FontSize', 20);
set(gca, 'LabelFontSizeMultiplier', 2.7)
FontS = 30;
set(gca, 'Fontsize',20) %increases fontsize
mo = 13;
ms = 13;
md = 13;
ma = 16;
...
plot(time1,Jw1,'o', 'MarkerSize', mo, 'MarkerEdgeColor', 'blue', 'MarkerFaceColor', [0 0 1])
hold on
plot(time2, Jw2,'square', 'MarkerSize', ms,  'MarkerEdgeColor', 'red', 'MarkerFaceColor', [1 0 0])
hold on
plot(time3, Jw3,'d', 'MarkerSize', md, 'MarkerEdgeColor', [0 .5 0], 'MarkerFaceColor', [0 .5 0])
get(gca)
ylabel('Jw (L/m^2-hr)', 'FontSize', FontS) %y axis label for graph on left
...
yyaxis right
plot(time1,Recovery1,'o', 'MarkerSize', mo, 'MarkerEdgeColor', 'blue')
hold on
plot(time2, Recovery2,'square', 'MarkerSize', ms,  'MarkerEdgeColor', 'red')
hold on
plot(time3, Recovery3,'d', 'MarkerSize', md, 'MarkerEdgeColor', [0 .5 0])
axis([0 tMax 0 100]) %range of x from 0 to 14 and y axis from 0 to 40
ylabel('Recovery (%)','FontSize', FontS) %labels y axis
grid on %adds grid to plot
xlabel('Time (hrs)','FontSize', FontS) %labels x axis
...
%Graph 2
ncond1 = [cond1(1), cond1(length(cond1-1))];
ncond2= [cond2(1), cond2(length(cond2-1))];
ncond3= [cond3(1), cond3(length(cond3-1))];
nrec1 = [Recovery1(1), Recovery1(length(Recovery1-1))];
nrec2 = [Recovery2(1), Recovery2(length(Recovery2-1))];
nrec3 = [Recovery3(1), Recovery3(length(Recovery3-1))];

fig = figure;
%ax = gca;
%set(gca, FontSizeMode, 'manual');
left_color = [0 0 0]; %makes color black
right_color = [0 0 0];
%ax.FontSize = 25;
set(fig,'defaultAxesColorOrder',[left_color; right_color]);
%set(gca, 'FontSize', 30);
set(gca, 'LabelFontSizeMultiplier', 2.5)

%left axis
yyaxis left %creates seperate axis to the left
plot(Recovery1,Jw1,'o', 'MarkerSize', mo, 'MarkerEdgeColor', 'blue', 'MarkerFaceColor', [0 0 1])
hold on
plot(Recovery2, Jw2,'square', 'MarkerSize', ms,  'MarkerEdgeColor', 'red', 'MarkerFaceColor', [1 0 0])
hold on
plot(Recovery3, Jw3,'d', 'MarkerSize', md, 'MarkerEdgeColor', [0 .5 0], 'MarkerFaceColor', [0 .5 0])
ylabel('Jw (L/m^2-hr)','FontSize', FontS) %y axis label for graph on left
axis([0 tMax 0 12]) %range of x from 0 to 16 and y axis from 0 to 10
hold on %Hold onto my previous command as other functions are added

yyaxis right
plot(nrec1,ncond1,'*', 'MarkerSize', ma, 'MarkerEdgeColor', 'blue')
hold on
plot(nrec2, ncond2,'*', 'MarkerSize', ma,  'MarkerEdgeColor', 'red')
hold on
plot(nrec3, ncond3,'*', 'MarkerSize', ma, 'MarkerEdgeColor', [0 .5 0])
axis([0 100 0 170]) %range of x from 0 to 14 and y axis from 0 to 40
ylabel('Conductivity (uS)','FontSize', FontS) %labels y axis
grid on %adds grid to plot
xlabel('Recovery Percent (%)','FontSize', FontS) %labels x axis
